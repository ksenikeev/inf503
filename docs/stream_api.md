...
### Optional
__Optional<T>__ - класс-обёртка для объекта типа __T__

`Optional` хранит объект, и предоставляет методы получения объекта и методы для обработки случаев когда объект пуст:
- `get` - получить объект (если объект пустой будет выброс исключения `NoSuchElementException`)
- `Optional.ofNullable(value)` - создает объект на базе значения `value` (может быть пустым)
- `isPresent()` - проверка наличия элемента
- `isEmpty()` - проверка отсутствия элемента
- `orElse(T other)` - если есть значение, то оно возвращается, иначе возвращается объект `other`
- `orElseThrow` - возвращает значение, если оно есть, иначе выбрасывается указанная ошибка

Так же имеются методы преобразования:
- `map`
- `stream`

## Преобразования в коллекции

Метод `collect(Collector collector)` - терминальный метод, создающий коллекцию из элементов стрима

Удобно использовать класс `Collectors`, содержащий методы, обеспечивающие создание коллекций разных типов:
- `toList`
- `toSet`
- `toMap`
- `groupingBy` - формирование Map в котором ключ - объект, по которому будут группироваться данные, значение - 
список (коллекция) элементов, относящихся к ключу
